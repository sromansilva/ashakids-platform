...
import { Video, Mic, MicOff, VideoOff, MessageCircle, FileText, Star, ChevronLeft, ChevronRight, Monitor, Phone, X, Check, Clock, Users, Maximize2, Minimize2, Volume2, Volume, Settings, Send, Paperclip, Download, Calendar, CheckCircle } from "lucide-react";
import { B, View, Btn, Crd, Bdg, Av, Ashi, Confetti, kids, appointments, msgs } from "./shared";

export const SESSION_THERAPIST = {
  name: "Dra. Ana Ruiz", specialty: "Terapia del Lenguaje",
  av: "AR", color: B.violet, date: "Miérc. 29 Jul 2026", time: "10:00 AM",
        ...
        <Crd className="p-6 mb-5 shadow-md">
          <div className="flex items-start gap-5 flex-wrap">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { icon: "📅", label: "Fecha",    val: t.date },
                  { icon: "⏰", label: "Hora",     val: t.time },
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex gap-2 sm:flex-col">
              {[
                { icon: "📅", label: "Cambiar horario" },
                { icon: "💬", label: "Enviar mensaje",  onClick: () => {} },
                { icon: "📄", label: "Ver materiales" },
              ].map(a => (
                <button key={a.label}
...
export function AshaSessionPrep({ go }: { go: (v: View) => void }) {
  ...
  const checklist = [
    ...
    { id: "noise", icon: "🔇", label: "Ambiente silencioso",hint: "Buscá un lugar tranquilo sin ruido de fondo" },
  ];
    ...
    <div style={{ background: "linear-gradient(180deg, #EFF6FF 0%, #FAFAF9 100%)", minHeight: "100vh", fontFamily: '"Nunito", system-ui, sans-serif' }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <Crd className="p-5 mb-5">
          ...
          <button className="mt-4 w-full flex items-center justify-center gap-2 py-3 rounded-2xl border-2 border-dashed border-violet-300 text-sm font-bold text-violet-600 hover:bg-violet-50 transition-colors">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
          ...
          <Crd className="p-5">
            ...
            <div className="flex flex-col gap-2">
              {[["🗣️","Pronunciación de la R"],["👂","Comprensión verbal"],["🎮","Juego interactivo"],["✋","Motricidad fina"]].map(([icon, label]) => (
        ...
        <div className="rounded-3xl p-7 text-center border-2 border-violet-200 mb-6"
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <button className="flex items-center justify-center gap-2 px-6 py-4 rounded-2xl border-2 border-violet-300 font-bold text-violet-700 hover:bg-violet-100 transition-colors text-sm">
              ⏳ Esperar al horario
            </button>
          </div>
        </div>
          ...
          <div className="w-80 flex-shrink-0 flex flex-col border-l" style={{ background: "#120A2E", borderColor: "rgba(255,255,255,0.08)" }}>
            ...
            {panel === "objectives" && (
              <div className="flex-1 p-4">
                ...
                {[["🗣️","Pronunciación de la R","En progreso"],["👂","Comprensión verbal","Completado"],["🎮","Juego interactivo","Pendiente"],["✋","Motricidad fina","Pendiente"]].map(([icon,label,status]) => (
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center text-center px-6 relative overflow-hidden"
      ...
      <div className="relative z-10 max-w-sm">
        ...
        {/* Rewards earned */}
        <div className="rounded-3xl p-5 mb-7 text-white" style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.2)" }}>
          <p className="text-xs font-black text-violet-300 uppercase tracking-widest mb-4">🏆 Recompensas ganadas</p>
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: "⭐", val: "+30",  label: "Estrellas" },
              { icon: "💎", val: "+150", label: "XP" },
              { icon: "🔥", val: "8",    label: "Racha días" },
            ].map(r => (
              <div key={r.label} className="rounded-2xl py-3 px-2" style={{ background: "rgba(255,255,255,0.1)" }}>
                <div className="text-3xl mb-1">{r.icon}</div>
                <p className="font-black text-white text-lg leading-none">{r.val}</p>
                <p className="text-xs text-violet-300 font-medium">{r.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 w-full">
          <button onClick={() => go("session/summary")}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-5">
          ...
          <Crd className="p-5">
            ...
            <div className="flex flex-col gap-2">
              {[["🗣️","Pronunciación de la R","completado"],["👂","Comprensión verbal","completado"],["🎮","Juego interactivo","completado"],["✋","Motricidad fina","pendiente"]].map(([icon,label,status]) => (
