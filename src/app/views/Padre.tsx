...
const PRICE_RANGES = ["Cualquier precio", "< S/ 45", "S/ 45–S/ 55", "> S/ 55"];
...
export function PadrePsicologos({ go, onRequest, bookedSlots = [] }: { go: (v: View) => void; onRequest: (request: AppointmentRequest) => void; bookedSlots?: { therapist: string; date: string; time: string }[] }) {
  ...
  const filtered = therapists.filter(t => {
    ...
    const matchPrice  = price === "Cualquier precio" ||
      (price === "< S/ 45" && t.price < 45) ||
      (price === "S/ 45–S/ 55" && t.price >= 45 && t.price <= 55) ||
      (price === "> S/ 55" && t.price > 55);
    return matchSearch && matchAvail && matchSpec && matchPrice;
  });
          ...
          <Crd key={t.id} className="p-5 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
            ...
            <div className="grid grid-cols-2 gap-2 mb-4 text-center">
              <div className="rounded-xl py-2" style={{ background: B.bg }}>
                <p className="text-xs text-[#9E95B7] font-bold">Desde</p>
                <p className="font-black text-[#1C1135] text-sm">S/ {t.price} / h</p>
              </div>
              <div className="rounded-xl py-2" style={{ background: B.bg }}>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              {[
                ...
                { label: "Tarifa", value: `S/ ${profile.price}/h` },
            ...
            <div className="p-6">
              <div className="rounded-2xl p-5 border border-[#E8E5F4] mb-5">
                ...
                {[
                  ...
                  { label: "Costo", value: `S/ ${booking.price}` },
                ].map(r => (
                ...
                {therapists.filter(t => t.available).map(t => (
                  <button key={t.id} onClick={() => setNewTherapist(t.name)}
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs font-extrabold" style={{ color: B.violet }}>S/ {t.price}/h</p>
                    </div>
                ...
                <>
                  <div className="rounded-2xl p-4 border border-[#E8E5F4] mb-5">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <p className="text-xs text-[#9E95B7] italic">Precios en definición · prototipo</p>
                    </div>
                  </div>
                  <div className="mb-5">
            ...
            return (
              <article key={apt.id} className="w-full min-h-[76px] rounded-2xl border border-[#E8E5F4] bg-white overflow-hidden transition-shadow hover:shadow-sm">
                ...
                {open && <div id={contentId} className="px-4 pb-4 pt-3 border-t border-[#F5F3FF] text-sm text-[#7C6F9A] font-medium animate-in fade-in slide-in-from-top-1 duration-200">
                  <div className="grid sm:grid-cols-2 gap-x-5 gap-y-2 py-1"><p><span className="font-bold text-[#1C1135]">Terapeuta:</span> {apt.therapist}</p><p><span className="font-bold text-[#1C1135]">Niño:</span> {apt.child}</p><p><span className="font-bold text-[#1C1135]">Fecha:</span> {apt.date}</p><p><span className="font-bold text-[#1C1135]">Hora y duración:</span> {apt.time} · 45 min</p><p><span className="font-bold text-[#1C1135]">Modalidad:</span> Virtual</p><p><span className="font-bold text-[#1C1135]">Referencia:</span> ASHA-{String(apt.id).slice(-6)}</p><p><span className="font-bold text-[#1C1135]">Precio:</span> Importe por definir</p><p><span className="font-bold text-[#1C1135]">Confirmación:</span> Confirmada por terapeuta</p></div>
